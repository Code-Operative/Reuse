...
class MAPOFSELLERS extends Module
{
	public function __construct()
	{
		$this->name = 'mapofsellers';
		$this->description = 'a map of the sellers on the site';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Mapofsellers_MONOLANGUAGE';
		$this->text_editor_content = 'Mapofsellers_TEXTEDITOR';
		$this->content_wrapper = 'Mapofsellers_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Mapofsellers_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Mapofsellers_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('map-of-sellers');
		$this->description = $this->l('a map of the sellers on the site');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !MAPOFSELLERSModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = MAPOFSELLERSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = MAPOFSELLERSModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					MAPOFSELLERSModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					MAPOFSELLERSModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																MAPOFSELLERSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class MAPOFSELLERSModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( MAPOFSELLERSModel::createFilesTable()
				&& MAPOFSELLERSModel::createContentTable());
	}
