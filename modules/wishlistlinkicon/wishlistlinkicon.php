...
class WISHLISTLINKICON extends Module
{
	public function __construct()
	{
		$this->name = 'wishlistlinkicon';
		$this->description = 'icon and link for wishlist in header';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Wishlistlinkicon_MONOLANGUAGE';
		$this->text_editor_content = 'Wishlistlinkicon_TEXTEDITOR';
		$this->content_wrapper = 'Wishlistlinkicon_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Wishlistlinkicon_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Wishlistlinkicon_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('wishlist-link-icon');
		$this->description = $this->l('icon and link for wishlist in header');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !WISHLISTLINKICONModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !WISHLISTLINKICONModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = WISHLISTLINKICONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = WISHLISTLINKICONModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					WISHLISTLINKICONModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					WISHLISTLINKICONModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																WISHLISTLINKICONModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class WISHLISTLINKICONModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'wishlistlinkicon',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( WISHLISTLINKICONModel::createFilesTable()
				&& WISHLISTLINKICONModel::createContentTable());
	}
