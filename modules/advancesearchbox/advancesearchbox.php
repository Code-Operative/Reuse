...
class ADVANCESEARCHBOX extends Module
{
	public function __construct()
	{
		$this->name = 'advancesearchbox';
		$this->description = 'This module is the design for the advance search box. ';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Advancesearchbox_MONOLANGUAGE';
		$this->text_editor_content = 'Advancesearchbox_TEXTEDITOR';
		$this->content_wrapper = 'Advancesearchbox_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Advancesearchbox_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Advancesearchbox_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('AdvanceSearchBox');
		$this->description = $this->l('This module is the design for the advance search box. ');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !ADVANCESEARCHBOXModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !ADVANCESEARCHBOXModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = ADVANCESEARCHBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = ADVANCESEARCHBOXModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					ADVANCESEARCHBOXModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					ADVANCESEARCHBOXModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																ADVANCESEARCHBOXModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class ADVANCESEARCHBOXModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'advancesearchbox',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( ADVANCESEARCHBOXModel::createFilesTable()
				&& ADVANCESEARCHBOXModel::createContentTable());
	}
