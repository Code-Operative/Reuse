...
class PRODUCTSHEADER extends Module
{
	public function __construct()
	{
		$this->name = 'productsheader';
		$this->description = 'header for products on page';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Productsheader_MONOLANGUAGE';
		$this->text_editor_content = 'Productsheader_TEXTEDITOR';
		$this->content_wrapper = 'Productsheader_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Productsheader_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Productsheader_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('products-header');
		$this->description = $this->l('header for products on page');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !PRODUCTSHEADERModel::createTables())
			return false;
	...
	public function uninstall()
	{
		$this->_clearCache('template.tpl');
		if (!parent::uninstall()
			|| !PRODUCTSHEADERModel::DropTables())
		{
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = PRODUCTSHEADERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = PRODUCTSHEADERModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					PRODUCTSHEADERModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					PRODUCTSHEADERModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																PRODUCTSHEADERModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class PRODUCTSHEADERModel extends ObjectModel
{

	public static $definition = array(
		'table' => 'productsheader',
		'primary' => 'file_id',
		'multishop' => true,
		'multilang' => true,
		'fields' => array(
	...
	public static function createTables()
	{
		...
		return ( PRODUCTSHEADERModel::createFilesTable()
				&& PRODUCTSHEADERModel::createContentTable());
	}
