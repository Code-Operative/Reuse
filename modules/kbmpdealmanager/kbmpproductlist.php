...
$id_seller = KbSeller::getSellerByCustomerId((int) Context::getContext()->customer->id);
...
$sql = 'SELECT p.`id_product`, `reference`, pl.name
        ...
        . ' INNER JOIN `' . _DB_PREFIX_ . 'kb_mp_seller_product` as kbsp ON (pl.id_product=kbsp.id_product)
        WHERE kbsp.id_seller='.(int) $id_seller.' and (pl.name LIKE \'%' . pSQL($query)
